...
from epistemic_confidence_engine import evaluate_epistemic_confidence
from knowledge_boundary_detector import detect_knowledge_boundaries
from ambiguity_escalation_engine import evaluate_ambiguity_escalation
from interpretation_conflict_engine import detect_interpretation_conflicts
from epistemic_freeze_controller import evaluate_epistemic_freeze_status
from canonical_intent_integrity_engine import validate_canonical_intent_integrity
from goal_drift_detector import detect_goal_drift
from mission_continuity_validator import validate_mission_continuity
from optimization_corruption_detector import detect_optimization_corruption
from teleological_freeze_controller import evaluate_teleological_freeze_status
...
ARCHITECTURE_LAYOUT = {
        ...
        "self_modification_boundary_engine.py",
            ...
            "epistemic_confidence_engine.py",
            "knowledge_boundary_detector.py",
            "ambiguity_escalation_engine.py",
            "interpretation_conflict_engine.py",
            "epistemic_freeze_controller.py",
            "canonical_intent_integrity_engine.py",
            "goal_drift_detector.py",
            "mission_continuity_validator.py",
            "optimization_corruption_detector.py",
            "teleological_freeze_controller.py",
        ],
    "extensions": [
...
def _register_system_planes():
    ...
    SYSTEM_PLANES = {
        "control": [
            ...
            "_build_epistemic_runtime_snapshot",
            "_build_teleological_runtime_snapshot",
            ...
            "epistemic_confidence_endpoint",
            "epistemic_boundaries_endpoint",
            "epistemic_conflicts_endpoint",
            "epistemic_freeze_status_endpoint",
            "teleology_integrity_endpoint",
            "teleology_drift_endpoint",
            "teleology_continuity_endpoint",
            "teleology_corruption_endpoint",
            "teleology_freeze_status_endpoint",
        ...
        "verification": [
            ...
            "evaluate_epistemic_confidence",
            "detect_knowledge_boundaries",
            "evaluate_ambiguity_escalation",
            "detect_interpretation_conflicts",
            "evaluate_epistemic_freeze_status",
            "validate_canonical_intent_integrity",
            "detect_goal_drift",
            "validate_mission_continuity",
            "detect_optimization_corruption",
            "evaluate_teleological_freeze_status",
            "get_audit",
...
def _build_epistemic_runtime_snapshot():
    screen_state = get_screen_state()
    capture = dict(screen_state.get("capture", {}) or {})
    ocr = dict(screen_state.get("ocr", {}) or {})
    ui_state = dict(screen_state.get("ui_state", {}) or {})
    ...
    truth_results = {}
    for replay_job_id in system_state.get("jobs", {}):
        truth_results[str(replay_job_id)] = resolve_truth(
            str(replay_job_id),
            system_state,
            authority_validation,
            chain_results.get(replay_job_id, {"valid": False, "reason": "missing_chain_result"})
        )

    jobs = system_state.get("jobs", {})
    governance_state = {
        "authority_valid": bool(authority_validation.get("valid", False)),
        "invariants_valid": bool(invariant_validation.get("valid", False)),
        "replay_anomaly_count": len(system_state.get("anomalies", [])),
    }
    execution_state = {
        "job_count": len(jobs),
        "executed_job_count": len([job for job in jobs.values() if bool(job.get("executed", False))]),
        "unknown_job_count": len([job for job in jobs.values() if str(job.get("status", "UNKNOWN")) == "UNKNOWN"]),
    }

    runtime_context = {
        "replay_anomaly_count": len(system_state.get("anomalies", [])),
        "unresolved_escalations": len(escalation_events),
...
def _build_teleological_runtime_snapshot():
    system_state = replay_system()
    runtime_status = get_runtime_status()
    failures = get_recent_failures(500).get("failures", [])
    metrics = get_runtime_metrics_snapshot(
        ...
        replay_state=system_state,
    )
    operator_trust = generate_operator_trust_snapshot(
        workflow_sessions=workflow_sessions,
        escalation_events=escalation_events,
    )
    ...
    return {
        ...
        "operational_traces": list(operational_traces),
        "contracts": contracts,
    }
...
@app.get("/epistemic/confidence")
def epistemic_confidence_endpoint():
    snapshot = _build_epistemic_runtime_snapshot()
    confidence = evaluate_epistemic_confidence(
        capture=snapshot.get("capture", {}),
        ocr=snapshot.get("ocr", {}),
        ui_state=snapshot.get("ui_state", {}),
        governance_state=snapshot.get("governance_state", {}),
        execution_state=snapshot.get("execution_state", {}),
    )
    return {
        "confidence": confidence,
        "context": {
            "ui_summary": _summarize_ui_state(snapshot.get("ui_state", {})),
            "job_count": snapshot.get("execution_state", {}).get("job_count", 0),
            "replay_anomaly_count": snapshot.get("governance_state", {}).get("replay_anomaly_count", 0),
        },
    }


@app.get("/epistemic/boundaries")
def epistemic_boundaries_endpoint():
    snapshot = _build_epistemic_runtime_snapshot()
    confidence = evaluate_epistemic_confidence(
        capture=snapshot.get("capture", {}),
        ocr=snapshot.get("ocr", {}),
    ...
    boundaries = detect_knowledge_boundaries(
...
@app.get("/epistemic/conflicts")
def epistemic_conflicts_endpoint():
    snapshot = _build_epistemic_runtime_snapshot()
    conflicts = detect_interpretation_conflicts(
        ui_state=snapshot.get("ui_state", {}),
        replay_state=snapshot.get("system_state", {}),
        authority_validation=snapshot.get("authority_validation", {}),
        truth_results=snapshot.get("truth_results", {}),
    )
    return {
        "conflicts": conflicts,
        "context": {
            "ui_summary": _summarize_ui_state(snapshot.get("ui_state", {})),
            "truth_job_count": len(snapshot.get("truth_results", {})),
        },
    }


@app.get("/epistemic/freeze-status")
def epistemic_freeze_status_endpoint():
    snapshot = _build_epistemic_runtime_snapshot()
    confidence = evaluate_epistemic_confidence(
        capture=snapshot.get("capture", {}),
        ocr=snapshot.get("ocr", {}),
    ...
    ambiguity = evaluate_ambiguity_escalation(
        confidence_result=confidence,
        boundary_result=boundaries,
        conflict_result=conflicts,
    )
    freeze_status = evaluate_epistemic_freeze_status(
        confidence_result=confidence,
        boundary_result=boundaries,
        conflict_result=conflicts,
...
@app.get("/teleology/integrity")
def teleology_integrity_endpoint():
    snapshot = _build_teleological_runtime_snapshot()
    integrity = validate_canonical_intent_integrity(
        canonical_specs=snapshot.get("canonical_specs", []),
        system_state=snapshot.get("system_state", {}),
        operational_traces=snapshot.get("operational_traces", []),
...
@app.get("/teleology/drift")
def teleology_drift_endpoint():
    snapshot = _build_teleological_runtime_snapshot()
    integrity = validate_canonical_intent_integrity(
        canonical_specs=snapshot.get("canonical_specs", []),
        system_state=snapshot.get("system_state", {}),
    ...
    drift = detect_goal_drift(
        ...
        operational_traces=snapshot.get("operational_traces", []),
        spec_history=snapshot.get("spec_history", []),
    )
    return {
        "integrity": integrity,
        "drift": drift,
...
@app.get("/teleology/continuity")
def teleology_continuity_endpoint():
    snapshot = _build_teleological_runtime_snapshot()
    integrity = validate_canonical_intent_integrity(
        canonical_specs=snapshot.get("canonical_specs", []),
        system_state=snapshot.get("system_state", {}),
    ...
    continuity = validate_mission_continuity(
        intent_integrity=integrity,
        goal_drift=drift,
        operator_trust=snapshot.get("operator_trust", {}),
...
@app.get("/teleology/corruption")
def teleology_corruption_endpoint():
    snapshot = _build_teleological_runtime_snapshot()
    integrity = validate_canonical_intent_integrity(
        canonical_specs=snapshot.get("canonical_specs", []),
        system_state=snapshot.get("system_state", {}),
    ...
    corruption = detect_optimization_corruption(
        goal_drift=drift,
        runtime_metrics=snapshot.get("runtime_metrics", {}),
        operational_traces=snapshot.get("operational_traces", []),
...
@app.get("/teleology/freeze-status")
def teleology_freeze_status_endpoint():
    snapshot = _build_teleological_runtime_snapshot()
    integrity = validate_canonical_intent_integrity(
        canonical_specs=snapshot.get("canonical_specs", []),
        system_state=snapshot.get("system_state", {}),
    ...
    freeze_status = evaluate_teleological_freeze_status(
        intent_integrity=integrity,
        goal_drift=drift,
        mission_continuity=continuity,
        optimization_corruption=corruption,
    )
    return {
        "integrity": integrity,
        "drift": drift,
